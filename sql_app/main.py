...
@app.get("/searchuser/")
def searchuser(email:str,db: Session = Depends(get_db)):
    db_user = crud.get_user_by_email(db, email=email)
    # return email
    if not db_user:
        raise HTTPException(status_code=404, detail="Email not registered")
    return db_user
#To find Data  email are available or not in our database
@app.get("/adduser/")
def adduser(email:str,db: Session = Depends(get_db)):
    db_user = crud.get_user_by_email(db, email=email)
    if  db_user:
        raise HTTPException(status_code=404, detail="Email already registered")
    
    db_user = crud.create_new_user(db,email=email)
    # return email
